...
class timeStringAxis(pg.AxisItem):
    def __init__(self, xs, strs, *args, **kwargs):
...
class LabelWindow(QMainWindow, Ui_MainWindow):
    ...
    def initEnv(self):
        ...
        self.workpath = os.path.join(os.path.dirname(__file__), "work")
    ...
    def initButton(self):
        ...
        self.class_n_b.clicked.connect(self.norm_c)
    ...
    def load_label(self, filename = "labels.txt"):
    ...
    def save_label(self, filename = None):
        if filename == None:
...
class LabelWindow_colored(LabelWindow):
    ...
    def showdata(self):
        ...
        self.data_plot.plot(xs, self.data, pen = pg.mkPen(width = 4, dash = (1, 2)),
...
if __name__ == '__main__':
    ...
    myWin = LabelWindow_colored(transform = norm_transform)
    myWin.show()
