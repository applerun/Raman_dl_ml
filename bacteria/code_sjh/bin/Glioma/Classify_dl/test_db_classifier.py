...
def main(
    ...
    indep_db = Raman_dirwise(r"D:\myPrograms\pythonProject\Raman_dl_ml\bacteria\data\脑胶质瘤\data_indep", mode = "all",
                             transform = db_cfg["transform"], LoadCsvFile = readdatafunc)
        ...
        for n in range(n_iter):
            for k in range(k_split):
                ...
                res = AST_main(
                    ...
                    **train_cfg,
                    k = k,
                    save_dir = os.path.join(coderoot, "checkpoints", "Glioma", "{}_{}_{}".format(net.model_name, n, k))
                )
                if type(train_db) == Raman_dirwise:
                    report_dirwise_classify(train_db, net, os.path.join(recordsubdir, "classfy_report_val.csv"),
                                            label2name = train_db.label2name())
                report_dirwise_classify(indep_db, net, os.path.join(recordsubdir, "unlabeled_classfy_report.csv"),
                                        label2name = train_db.label2name())
    ...
    for ele in eles:  # TODO：选择合适的标签
        ...
        for preprocess in [
            ...
            main(dataroot, db_cfg = db_cfg, raman = raman, modellist = modellist, recorddir = recorddir,
                 path2labelfunc = path2labelfunc, train_cfg = train_cfg)
