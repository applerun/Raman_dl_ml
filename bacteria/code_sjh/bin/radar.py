...
from bacteria.code_sjh.utils.Validation import evaluate, evaluate_labelwise
...
def AST_main(
		sample_tensor,  # TODO:自动收集sample tensor
		train_db,
		val_db,
		test_db,  # TODO:选择分配比例和数据文件夹
		device,  # 选择训练键
		lr = 0.002,  # 选择learning rate
		epochs = 100,  # 选择epoch
		batchsz = 40,  # 选择batch size
		numworkers = 0,
		modelname = None,  # 选择保存名
		vis = None,
		save_dir = os.path.join(coderoot, "checkpoints", ),  # 选择保存目录
		model = AlexNet_Sun,  # 选择分类模型
		epo_interv = 2,  # 每隔epo_interv 验证一次
		# prog_bar = False,  # 实现进度条功能
		criteon = nn.CrossEntropyLoss(),  # 选择criteon
		lr_decay_rate = 0.5,
		lr_decay_period = 60,
):
	alexnet = model(tensor_sample = sample_tensor, num_classes = test_db.num_classes()).to(device)  # 生成模型
