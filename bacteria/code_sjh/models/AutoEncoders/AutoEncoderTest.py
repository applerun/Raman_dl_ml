        ...
        if (epoch % 20 == 9 or epoch == epochs - 1) and epoch > 30:
            ...
            with torch.no_grad():
                for idx in range(len(label2name_val.keys())):
                    ...
                    visdom_func.spectrum_vis(spectrum, xs, win, name = "x", vis = vis)
                    visdom_func.spectrum_vis(spectrum_hat, xs, win, update = "append", name = "x_hat", vis = vis)
                    if neck_axis < 4:
...
def mae_test(savefilemode = "combined",
    ...
    y_train = torch.squeeze(torch.stack(train_db.numclasses, dim = 0)).numpy()
    ...
    y_val = torch.squeeze(torch.stack(val_db.numclasses, dim = 0)).numpy()
