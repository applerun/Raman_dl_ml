...
class AlexNet_Sun(BasicModule):
	'''
	参考《基于拉曼光谱技术和深度学习算法的细菌鉴定》-孙宏奕
	'''

	def __init__(self, sample_tensor: torch.Tensor, num_classes = 6, dropout = 0.1, dense_bias = True, alpha = 0.15):
		"""
		...
		"""
		self.alpha = alpha
		super(AlexNet_Sun, self).__init__()
...
if __name__ == '__main__':
	...
	for label in spectrum_each_label.keys():
		...
		for pred_i in spectrum_each_label.keys():
			...
			visdom_utils.spectrum_vis(spectrum, test_db.wavelengths(),
	...
	with torch.no_grad():
		...
		for l in range(16):
			...
			m_0, _, _ = visdom_utils.data2mean_std(s_0)
			m_1, _, _ = visdom_utils.data2mean_std(s_1)
			r, p = scipy.stats.pearsonr(m_0, m_1)
			pearsons[0].append(r)

			pearsons[1].append(p)

			stat, pval, mgcdict = scipy.stats.multiscale_graphcorr(s_0.numpy(), s_1.numpy(), is_twosamp = True)
			...
			for idx in range(s_0.shape[-1]):
				r, t = scipy.stats.ttest_ind(s_0[:, idx].numpy(), s_1[:, idx].numpy(), equal_var = False)
				t_test[idx] = abs(r)
